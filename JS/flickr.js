...
/*
 *  'photoLinks()' will be called in the second conditional of 'getPhoto()'
...
async function getPhoto(lat, lon)
{
  try
  {
    ...
    if (response[1].ok) {response[2] = await getInfoLink(response[1])};

    // waiting for the two URLs of the photos
    if (response[2].ok) {return await photoLinks(response[2])};
